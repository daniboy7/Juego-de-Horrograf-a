...
public class PartidaNivel4 extends AppCompatActivity {
    ...
    private int batHeightSize;
    private int batWidthSize;

    AnimationDrawable flying;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_partida_nivel4);

        mp = MediaPlayer.create(this, R.raw.pet_park);
        mp.setLooping(true);
        mp.seekTo(0);
        ...
        bat.setBackgroundResource(R.drawable.flyingbat);
        flying =(AnimationDrawable)bat.getBackground();
        flying.start();
            ...
            public void onClick(View view) {
                if (!isAnswered()){
                    if(!(respuesta.getText().toString().isEmpty())){
                        ...
                        Toast.makeText(PartidaNivel4.this,"Por favor ingrese una respuesta",Toast.LENGTH_SHORT).show();
        ...
        if (lapCounter <= lapCountTotal){
            if(itemCounter < itemCountTotal){
                ...
                if (getErrores() < getCurrentItem().getErrores_no_permitidos()){
                    ...
                    Intent gameover = new Intent(PartidaNivel4.this,GameOver.class);
                    ...
                    PartidaNivel4.this.startActivity(gameover);
                    PartidaNivel4.this.finish();
                }
            ...
            Intent tableroPuntuacion = new Intent(PartidaNivel4.this,Puntuacion.class);
            ...
            PartidaNivel4.this.startActivity(tableroPuntuacion);
            PartidaNivel4.this.finish();
        }
    ...
    private void getData(int etapa, int nivel) {
        ...
        RequestQueue cola = Volley.newRequestQueue(PartidaNivel4.this);
    ...
    private void checkBatPosition() {
        frameHeight = frame.getHeight();
        frameWidth = frame.getWidth();
        batHeightSize = bat.getHeight();
        batWidthSize = bat.getWidth();

        if (batY < 0)
            batY = 0;
        if (batY > frameHeight - batHeightSize)
            batY = frameHeight - batHeightSize;
        if (batX < 0)
            batX = 0;
        if (batX > frameWidth - batWidthSize)
            batX = frameWidth - batWidthSize;
